// GENERATED by :codegen from ICU release-78.3. Do not edit.
// Regenerate with: ./gradlew :codegen:generateLocaleData
package dev.carcara.kotlinx.locale.currency

internal const val ICU_VERSION: String = "release-78.3"

internal class IcuCurrencyGolden(
    val tag: String,
    val symbols: Map<String, String>,
    val names: Map<String, String>,
    val decimal: String?,
    val group: String?,
    val currencyPattern: String?,
)

internal val icuCurrencyGoldenData: List<IcuCurrencyGolden> = listOf(
    IcuCurrencyGolden(
        tag = "en",
        symbols = mapOf(
            "USD" to "\$",
            "EUR" to "€",
            "JPY" to "¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "US Dollar",
            "EUR" to "Euro",
            "JPY" to "Japanese Yen",
            "GBP" to "British Pound",
            "BRL" to "Brazilian Real",
            "CHF" to "Swiss Franc",
            "INR" to "Indian Rupee",
            "CNY" to "Chinese Yuan",
            "RUB" to "Russian Ruble",
            "EGP" to "Egyptian Pound",
            "MXN" to "Mexican Peso",
            "KRW" to "South Korean Won",
            "TRY" to "Turkish Lira",
            "ZAR" to "South African Rand",
            "SAR" to "Saudi Riyal",
            "ARS" to "Argentine Peso",
            "UAH" to "Ukrainian Hryvnia",
        ),
        decimal = ".",
        group = ",",
        currencyPattern = "¤#,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "en-GB",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "JP¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "US Dollar",
            "EUR" to "Euro",
            "JPY" to "Japanese Yen",
            "GBP" to "British Pound",
            "BRL" to "Brazilian Real",
            "CHF" to "Swiss Franc",
            "INR" to "Indian Rupee",
            "CNY" to "Chinese Yuan",
            "RUB" to "Russian Rouble",
            "EGP" to "Egyptian Pound",
            "MXN" to "Mexican Peso",
            "KRW" to "South Korean Won",
            "TRY" to "Turkish Lira",
            "ZAR" to "South African Rand",
            "SAR" to "Saudi Riyal",
            "ARS" to "Argentine Peso",
            "UAH" to "Ukrainian Hryvnia",
        ),
        decimal = ".",
        group = ",",
        currencyPattern = "¤#,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "en-AU",
        symbols = mapOf(
            "USD" to "USD",
            "EUR" to "EUR",
            "JPY" to "JPY",
            "GBP" to "GBP",
            "BRL" to "BRL",
            "CHF" to "CHF",
            "INR" to "INR",
            "CNY" to "CNY",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MXN",
            "KRW" to "KRW",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "US Dollar",
            "EUR" to "Euro",
            "JPY" to "Japanese Yen",
            "GBP" to "British Pound",
            "BRL" to "Brazilian Real",
            "CHF" to "Swiss Franc",
            "INR" to "Indian Rupee",
            "CNY" to "Chinese Yuan",
            "RUB" to "Russian Rouble",
            "EGP" to "Egyptian Pound",
            "MXN" to "Mexican Peso",
            "KRW" to "South Korean Won",
            "TRY" to "Turkish Lira",
            "ZAR" to "South African Rand",
            "SAR" to "Saudi Riyal",
            "ARS" to "Argentine Peso",
            "UAH" to "Ukrainian Hryvnia",
        ),
        decimal = ".",
        group = ",",
        currencyPattern = "¤#,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "de",
        symbols = mapOf(
            "USD" to "\$",
            "EUR" to "€",
            "JPY" to "¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "US-Dollar",
            "EUR" to "Euro",
            "JPY" to "Japanischer Yen",
            "GBP" to "Britisches Pfund",
            "BRL" to "Brasilianischer Real",
            "CHF" to "Schweizer Franken",
            "INR" to "Indische Rupie",
            "CNY" to "Renminbi Yuan",
            "RUB" to "Russischer Rubel",
            "EGP" to "Ägyptisches Pfund",
            "MXN" to "Mexikanischer Peso",
            "KRW" to "Südkoreanischer Won",
            "TRY" to "Türkische Lira",
            "ZAR" to "Südafrikanischer Rand",
            "SAR" to "Saudi-Rial",
            "ARS" to "Argentinischer Peso",
            "UAH" to "Ukrainische Hrywnja",
        ),
        decimal = ",",
        group = ".",
        currencyPattern = "#,##0.00 ¤",
    ),
    IcuCurrencyGolden(
        tag = "fr",
        symbols = mapOf(
            "USD" to "\$US",
            "EUR" to "€",
            "JPY" to "JPY",
            "GBP" to "£GB",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CNY",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "\$MX",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "\$AR",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "dollar des États-Unis",
            "EUR" to "euro",
            "JPY" to "yen japonais",
            "GBP" to "livre sterling",
            "BRL" to "réal brésilien",
            "CHF" to "franc suisse",
            "INR" to "roupie indienne",
            "CNY" to "yuan renminbi chinois",
            "RUB" to "rouble russe",
            "EGP" to "livre égyptienne",
            "MXN" to "peso mexicain",
            "KRW" to "won sud-coréen",
            "TRY" to "livre turque",
            "ZAR" to "rand sud-africain",
            "SAR" to "riyal saoudien",
            "ARS" to "peso argentin",
            "UAH" to "hryvnia ukrainienne",
        ),
        decimal = ",",
        group = " ",
        currencyPattern = "#,##0.00 ¤",
    ),
    IcuCurrencyGolden(
        tag = "fr-CA",
        symbols = mapOf(
            "USD" to "\$ US",
            "EUR" to "€",
            "JPY" to "¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "INR",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MXN",
            "KRW" to "KRW",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "dollar des États-Unis",
            "EUR" to "euro",
            "JPY" to "yen japonais",
            "GBP" to "livre sterling",
            "BRL" to "réal brésilien",
            "CHF" to "franc suisse",
            "INR" to "roupie indienne",
            "CNY" to "yuan renminbi chinois",
            "RUB" to "rouble russe",
            "EGP" to "livre égyptienne",
            "MXN" to "peso mexicain",
            "KRW" to "won sud-coréen",
            "TRY" to "livre turque",
            "ZAR" to "rand sud-africain",
            "SAR" to "rial saoudien",
            "ARS" to "peso argentin",
            "UAH" to "hryvnia ukrainienne",
        ),
        decimal = ",",
        group = " ",
        currencyPattern = "#,##0.00 ¤",
    ),
    IcuCurrencyGolden(
        tag = "es",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "JPY",
            "GBP" to "GBP",
            "BRL" to "BRL",
            "CHF" to "CHF",
            "INR" to "INR",
            "CNY" to "CNY",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MXN",
            "KRW" to "KRW",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "dólar estadounidense",
            "EUR" to "euro",
            "JPY" to "yen japonés",
            "GBP" to "libra esterlina",
            "BRL" to "real brasileño",
            "CHF" to "franco suizo",
            "INR" to "rupia india",
            "CNY" to "yuan renminbi",
            "RUB" to "rublo ruso",
            "EGP" to "libra egipcia",
            "MXN" to "peso mexicano",
            "KRW" to "won surcoreano",
            "TRY" to "lira turca",
            "ZAR" to "rand sudafricano",
            "SAR" to "rial saudí",
            "ARS" to "peso argentino",
            "UAH" to "grivna ucraniana",
        ),
        decimal = ",",
        group = ".",
        currencyPattern = "#,##0.00 ¤",
    ),
    IcuCurrencyGolden(
        tag = "es-MX",
        symbols = mapOf(
            "USD" to "USD",
            "EUR" to "EUR",
            "JPY" to "JPY",
            "GBP" to "GBP",
            "BRL" to "BRL",
            "CHF" to "CHF",
            "INR" to "INR",
            "CNY" to "CNY",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "\$",
            "KRW" to "KRW",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "dólar estadounidense",
            "EUR" to "euro",
            "JPY" to "yen japonés",
            "GBP" to "libra esterlina",
            "BRL" to "real brasileño",
            "CHF" to "franco suizo",
            "INR" to "rupia india",
            "CNY" to "yuan renminbi",
            "RUB" to "rublo ruso",
            "EGP" to "libra egipcia",
            "MXN" to "peso mexicano",
            "KRW" to "won surcoreano",
            "TRY" to "lira turca",
            "ZAR" to "rand sudafricano",
            "SAR" to "rial saudí",
            "ARS" to "peso argentino",
            "UAH" to "grivna ucraniana",
        ),
        decimal = ".",
        group = ",",
        currencyPattern = "¤#,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "it",
        symbols = mapOf(
            "USD" to "USD",
            "EUR" to "€",
            "JPY" to "JPY",
            "GBP" to "£",
            "BRL" to "BRL",
            "CHF" to "CHF",
            "INR" to "INR",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MXN",
            "KRW" to "KRW",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "dollaro statunitense",
            "EUR" to "euro",
            "JPY" to "yen giapponese",
            "GBP" to "sterlina britannica",
            "BRL" to "real brasiliano",
            "CHF" to "franco svizzero",
            "INR" to "rupia indiana",
            "CNY" to "yuan cinese",
            "RUB" to "rublo russo",
            "EGP" to "sterlina egiziana",
            "MXN" to "peso messicano",
            "KRW" to "won sudcoreano",
            "TRY" to "lira turca",
            "ZAR" to "rand sudafricano",
            "SAR" to "riyal saudita",
            "ARS" to "peso argentino",
            "UAH" to "grivnia ucraina",
        ),
        decimal = ",",
        group = ".",
        currencyPattern = "#,##0.00 ¤",
    ),
    IcuCurrencyGolden(
        tag = "pt",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "JP¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "Dólar americano",
            "EUR" to "Euro",
            "JPY" to "Iene japonês",
            "GBP" to "Libra esterlina",
            "BRL" to "Real brasileiro",
            "CHF" to "Franco suíço",
            "INR" to "Rupia indiana",
            "CNY" to "Yuan chinês",
            "RUB" to "Rublo russo",
            "EGP" to "Libra egípcia",
            "MXN" to "Peso mexicano",
            "KRW" to "Won sul-coreano",
            "TRY" to "Lira turca",
            "ZAR" to "Rand sul-africano",
            "SAR" to "Riyal saudita",
            "ARS" to "Peso argentino",
            "UAH" to "Hryvnia ucraniano",
        ),
        decimal = ",",
        group = ".",
        currencyPattern = "¤ #,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "pt-PT",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "JP¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "dólar dos Estados Unidos",
            "EUR" to "euro",
            "JPY" to "iene japonês",
            "GBP" to "libra esterlina britânica",
            "BRL" to "real brasileiro",
            "CHF" to "franco suíço",
            "INR" to "rupia indiana",
            "CNY" to "yuan",
            "RUB" to "rublo russo",
            "EGP" to "libra egípcia",
            "MXN" to "peso mexicano",
            "KRW" to "won sul-coreano",
            "TRY" to "lira turca",
            "ZAR" to "rand sul-africano",
            "SAR" to "rial saudita",
            "ARS" to "peso argentino",
            "UAH" to "hryvnia ucraniano",
        ),
        decimal = ",",
        group = " ",
        currencyPattern = "#,##0.00 ¤",
    ),
    IcuCurrencyGolden(
        tag = "ja",
        symbols = mapOf(
            "USD" to "\$",
            "EUR" to "€",
            "JPY" to "￥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "元",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "米ドル",
            "EUR" to "ユーロ",
            "JPY" to "日本円",
            "GBP" to "英国ポンド",
            "BRL" to "ブラジル レアル",
            "CHF" to "スイス フラン",
            "INR" to "インド ルピー",
            "CNY" to "中国人民元",
            "RUB" to "ロシア ルーブル",
            "EGP" to "エジプト ポンド",
            "MXN" to "メキシコ ペソ",
            "KRW" to "韓国ウォン",
            "TRY" to "トルコ リラ",
            "ZAR" to "南アフリカ ランド",
            "SAR" to "サウジ リヤル",
            "ARS" to "アルゼンチン ペソ",
            "UAH" to "ウクライナ フリヴニャ",
        ),
        decimal = ".",
        group = ",",
        currencyPattern = "¤#,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "ko",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "JP¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "미국 달러",
            "EUR" to "유로",
            "JPY" to "일본 엔화",
            "GBP" to "영국 파운드",
            "BRL" to "브라질 레알",
            "CHF" to "스위스 프랑",
            "INR" to "인도 루피",
            "CNY" to "중국 위안화",
            "RUB" to "러시아 루블",
            "EGP" to "이집트 파운드",
            "MXN" to "멕시코 페소",
            "KRW" to "대한민국 원",
            "TRY" to "튀르키예 리라",
            "ZAR" to "남아프리카 랜드",
            "SAR" to "사우디아라비아 리얄",
            "ARS" to "아르헨티나 페소",
            "UAH" to "우크라이나 그리브나",
        ),
        decimal = ".",
        group = ",",
        currencyPattern = "¤#,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "ru",
        symbols = mapOf(
            "USD" to "\$",
            "EUR" to "€",
            "JPY" to "¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "₽",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "₴",
        ),
        names = mapOf(
            "USD" to "доллар США",
            "EUR" to "евро",
            "JPY" to "японская иена",
            "GBP" to "британский фунт стерлингов",
            "BRL" to "бразильский реал",
            "CHF" to "швейцарский франк",
            "INR" to "индийская рупия",
            "CNY" to "китайский юань",
            "RUB" to "российский рубль",
            "EGP" to "египетский фунт",
            "MXN" to "мексиканский песо",
            "KRW" to "южнокорейская вона",
            "TRY" to "турецкая лира",
            "ZAR" to "южноафриканский рэнд",
            "SAR" to "саудовский риял",
            "ARS" to "аргентинский песо",
            "UAH" to "украинская гривна",
        ),
        decimal = ",",
        group = " ",
        currencyPattern = "#,##0.00 ¤",
    ),
    IcuCurrencyGolden(
        tag = "ar",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "JP¥",
            "GBP" to "UK£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "ج.م.‏",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "ر.س.‏",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "دولار أمريكي",
            "EUR" to "يورو",
            "JPY" to "ين ياباني",
            "GBP" to "جنيه إسترليني",
            "BRL" to "ريال برازيلي",
            "CHF" to "فرنك سويسري",
            "INR" to "روبية هندي",
            "CNY" to "يوان صيني",
            "RUB" to "روبل روسي",
            "EGP" to "جنيه مصري",
            "MXN" to "بيزو مكسيكي",
            "KRW" to "وون كوريا الجنوبية",
            "TRY" to "ليرة تركية",
            "ZAR" to "راند جنوب أفريقيا",
            "SAR" to "ريال سعودي",
            "ARS" to "بيزو أرجنتيني",
            "UAH" to "هريفنيا أوكراني",
        ),
        decimal = ".",
        group = ",",
        currencyPattern = "‏#,##0.00 ¤;‏-#,##0.00 ¤",
    ),
    IcuCurrencyGolden(
        tag = "fi",
        symbols = mapOf(
            "USD" to "\$",
            "EUR" to "€",
            "JPY" to "¥",
            "GBP" to "£",
            "BRL" to "BRL",
            "CHF" to "CHF",
            "INR" to "INR",
            "CNY" to "CNY",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MXN",
            "KRW" to "KRW",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "Yhdysvaltain dollari",
            "EUR" to "euro",
            "JPY" to "Japanin jeni",
            "GBP" to "Englannin punta",
            "BRL" to "Brasilian real",
            "CHF" to "Sveitsin frangi",
            "INR" to "Intian rupia",
            "CNY" to "Kiinan juan",
            "RUB" to "Venäjän rupla",
            "EGP" to "Egyptin punta",
            "MXN" to "Meksikon peso",
            "KRW" to "Etelä-Korean won",
            "TRY" to "Turkin liira",
            "ZAR" to "Etelä-Afrikan randi",
            "SAR" to "Saudi-Arabian rial",
            "ARS" to "Argentiinan peso",
            "UAH" to "Ukrainan hryvnia",
        ),
        decimal = ",",
        group = " ",
        currencyPattern = "#,##0.00 ¤",
    ),
    IcuCurrencyGolden(
        tag = "pl",
        symbols = mapOf(
            "USD" to "USD",
            "EUR" to "€",
            "JPY" to "JPY",
            "GBP" to "GBP",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "INR",
            "CNY" to "CNY",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MXN",
            "KRW" to "KRW",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "dolar amerykański",
            "EUR" to "euro",
            "JPY" to "jen japoński",
            "GBP" to "funt szterling",
            "BRL" to "real brazylijski",
            "CHF" to "frank szwajcarski",
            "INR" to "rupia indyjska",
            "CNY" to "juan chiński",
            "RUB" to "rubel rosyjski",
            "EGP" to "funt egipski",
            "MXN" to "peso meksykańskie",
            "KRW" to "won południowokoreański",
            "TRY" to "lira turecka",
            "ZAR" to "rand południowoafrykański",
            "SAR" to "rial saudyjski",
            "ARS" to "peso argentyńskie",
            "UAH" to "hrywna ukraińska",
        ),
        decimal = ",",
        group = " ",
        currencyPattern = "#,##0.00 ¤",
    ),
    IcuCurrencyGolden(
        tag = "tr",
        symbols = mapOf(
            "USD" to "\$",
            "EUR" to "€",
            "JPY" to "¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "₺",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "ABD doları",
            "EUR" to "Euro",
            "JPY" to "Japon yeni",
            "GBP" to "İngiliz sterlini",
            "BRL" to "Brezilya reali",
            "CHF" to "İsviçre frangı",
            "INR" to "Hindistan rupisi",
            "CNY" to "Çin yuanı",
            "RUB" to "Rus rublesi",
            "EGP" to "Mısır lirası",
            "MXN" to "Meksika pesosu",
            "KRW" to "Güney Kore wonu",
            "TRY" to "Türk lirası",
            "ZAR" to "Güney Afrika randı",
            "SAR" to "Suudi Arabistan riyali",
            "ARS" to "Arjantin pesosu",
            "UAH" to "Ukrayna grivnası",
        ),
        decimal = ",",
        group = ".",
        currencyPattern = "¤#,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "nl",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "JP¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "Amerikaanse dollar",
            "EUR" to "Euro",
            "JPY" to "Japanse yen",
            "GBP" to "Britse pond",
            "BRL" to "Braziliaanse real",
            "CHF" to "Zwitserse frank",
            "INR" to "Indiase roepie",
            "CNY" to "Chinese yuan",
            "RUB" to "Russische roebel",
            "EGP" to "Egyptisch pond",
            "MXN" to "Mexicaanse peso",
            "KRW" to "Zuid-Koreaanse won",
            "TRY" to "Turkse lira",
            "ZAR" to "Zuid-Afrikaanse rand",
            "SAR" to "Saoedi-Arabische riyal",
            "ARS" to "Argentijnse peso",
            "UAH" to "Oekraïense hryvnia",
        ),
        decimal = ",",
        group = ".",
        currencyPattern = "¤ #,##0.00;¤ -#,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "th",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "ดอลลาร์สหรัฐ",
            "EUR" to "ยูโร",
            "JPY" to "เยนญี่ปุ่น",
            "GBP" to "ปอนด์สเตอร์ลิง (สหราชอาณาจักร)",
            "BRL" to "เรียลบราซิล",
            "CHF" to "ฟรังก์สวิส",
            "INR" to "รูปีอินเดีย",
            "CNY" to "หยวนจีน",
            "RUB" to "รูเบิลรัสเซีย",
            "EGP" to "ปอนด์อียิปต์",
            "MXN" to "เปโซเม็กซิโก",
            "KRW" to "วอนเกาหลีใต้",
            "TRY" to "ลีราตุรกี",
            "ZAR" to "แรนด์แอฟริกาใต้",
            "SAR" to "ริยัลซาอุดีอาระเบีย",
            "ARS" to "เปโซอาร์เจนตินา",
            "UAH" to "ฮรีฟเนียยูเครน",
        ),
        decimal = ".",
        group = ",",
        currencyPattern = "¤#,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "zh",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "JP¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "美元",
            "EUR" to "欧元",
            "JPY" to "日元",
            "GBP" to "英镑",
            "BRL" to "巴西雷亚尔",
            "CHF" to "瑞士法郎",
            "INR" to "印度卢比",
            "CNY" to "人民币",
            "RUB" to "俄罗斯卢布",
            "EGP" to "埃及镑",
            "MXN" to "墨西哥比索",
            "KRW" to "韩元",
            "TRY" to "土耳其里拉",
            "ZAR" to "南非兰特",
            "SAR" to "沙特里亚尔",
            "ARS" to "阿根廷比索",
            "UAH" to "乌克兰格里夫纳",
        ),
        decimal = ".",
        group = ",",
        currencyPattern = "¤#,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "zh-Hant",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "￦",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "美元",
            "EUR" to "歐元",
            "JPY" to "日圓",
            "GBP" to "英鎊",
            "BRL" to "巴西雷亞爾",
            "CHF" to "瑞士法郎",
            "INR" to "印度盧比",
            "CNY" to "人民幣",
            "RUB" to "俄羅斯盧布",
            "EGP" to "埃及鎊",
            "MXN" to "墨西哥披索",
            "KRW" to "韓元",
            "TRY" to "土耳其里拉",
            "ZAR" to "南非蘭特",
            "SAR" to "沙烏地里亞爾",
            "ARS" to "阿根廷披索",
            "UAH" to "烏克蘭格里夫納",
        ),
        decimal = ".",
        group = ",",
        currencyPattern = "¤#,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "hi",
        symbols = mapOf(
            "USD" to "\$",
            "EUR" to "€",
            "JPY" to "JP¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "यूएस डॉलर",
            "EUR" to "यूरो",
            "JPY" to "जापानी येन",
            "GBP" to "ब्रिटिश पाउंड स्टर्लिंग",
            "BRL" to "ब्राज़ीली रियाल",
            "CHF" to "स्विस फ़्रैंक",
            "INR" to "भारतीय रुपया",
            "CNY" to "चीनी युआन",
            "RUB" to "रूसी रूबल",
            "EGP" to "मिस्र पाउंड",
            "MXN" to "मैक्सिकन पेसो",
            "KRW" to "दक्षिण कोरियाई वॉन",
            "TRY" to "तुर्की लीरा",
            "ZAR" to "दक्षिण अफ़्रीकी रैंड",
            "SAR" to "सउदी रियाल",
            "ARS" to "अर्जेंटीनी पेसो",
            "UAH" to "यूक्रेनियन रिव्निया",
        ),
        decimal = ".",
        group = ",",
        currencyPattern = "¤#,##,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "cs",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "JP¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "INR",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "americký dolar",
            "EUR" to "euro",
            "JPY" to "japonský jen",
            "GBP" to "britská libra",
            "BRL" to "brazilský real",
            "CHF" to "švýcarský frank",
            "INR" to "indická rupie",
            "CNY" to "čínský jüan",
            "RUB" to "ruský rubl",
            "EGP" to "egyptská libra",
            "MXN" to "mexické peso",
            "KRW" to "jihokorejský won",
            "TRY" to "turecká lira",
            "ZAR" to "jihoafrický rand",
            "SAR" to "saúdský rijál",
            "ARS" to "argentinské peso",
            "UAH" to "ukrajinská hřivna",
        ),
        decimal = ",",
        group = " ",
        currencyPattern = "#,##0.00 ¤",
    ),
    IcuCurrencyGolden(
        tag = "sv",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "JPY",
            "GBP" to "GBP",
            "BRL" to "BR\$",
            "CHF" to "CHF",
            "INR" to "INR",
            "CNY" to "CNY",
            "RUB" to "RUB",
            "EGP" to "EG£",
            "MXN" to "MX\$",
            "KRW" to "KRW",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "amerikansk dollar",
            "EUR" to "euro",
            "JPY" to "japansk yen",
            "GBP" to "brittiskt pund",
            "BRL" to "brasiliansk real",
            "CHF" to "schweizisk franc",
            "INR" to "indisk rupie",
            "CNY" to "kinesisk yuan",
            "RUB" to "rysk rubel",
            "EGP" to "egyptiskt pund",
            "MXN" to "mexikansk peso",
            "KRW" to "sydkoreansk won",
            "TRY" to "turkisk lira",
            "ZAR" to "sydafrikansk rand",
            "SAR" to "saudisk riyal",
            "ARS" to "argentinsk peso",
            "UAH" to "ukrainsk hryvnia",
        ),
        decimal = ",",
        group = " ",
        currencyPattern = "#,##0.00 ¤",
    ),
    IcuCurrencyGolden(
        tag = "uk",
        symbols = mapOf(
            "USD" to "USD",
            "EUR" to "EUR",
            "JPY" to "¥",
            "GBP" to "GBP",
            "BRL" to "BRL",
            "CHF" to "CHF",
            "INR" to "INR",
            "CNY" to "CNY",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MXN",
            "KRW" to "KRW",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "₴",
        ),
        names = mapOf(
            "USD" to "долар США",
            "EUR" to "євро",
            "JPY" to "японська єна",
            "GBP" to "англійський фунт",
            "BRL" to "бразильський реал",
            "CHF" to "швейцарський франк",
            "INR" to "індійська рупія",
            "CNY" to "китайський юань",
            "RUB" to "російський рубль",
            "EGP" to "єгипетський фунт",
            "MXN" to "мексиканський песо",
            "KRW" to "південнокорейський вон",
            "TRY" to "турецька ліра",
            "ZAR" to "південноафриканський ранд",
            "SAR" to "саудівський ріал",
            "ARS" to "аргентинський песо",
            "UAH" to "українська гривня",
        ),
        decimal = ",",
        group = " ",
        currencyPattern = "#,##0.00 ¤",
    ),
    IcuCurrencyGolden(
        tag = "he",
        symbols = mapOf(
            "USD" to "\$",
            "EUR" to "€",
            "JPY" to "¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "‎CN¥‎",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "דולר אמריקאי",
            "EUR" to "אירו",
            "JPY" to "ין יפני",
            "GBP" to "לירה שטרלינג",
            "BRL" to "ריאל ברזילאי",
            "CHF" to "פרנק שוויצרי",
            "INR" to "רופי הודי",
            "CNY" to "יואן סיני",
            "RUB" to "רובל רוסי",
            "EGP" to "לירה מצרית",
            "MXN" to "פסו מקסיקני",
            "KRW" to "וון דרום קוריאני",
            "TRY" to "לירה טורקית חדשה",
            "ZAR" to "ראנד דרום אפריקאי",
            "SAR" to "ריאל סעודי",
            "ARS" to "פסו ארגנטינאי",
            "UAH" to "הריבנה אוקראיני",
        ),
        decimal = ".",
        group = ",",
        currencyPattern = "‏#,##0.00 ‏¤;‏-#,##0.00 ‏¤",
    ),
    IcuCurrencyGolden(
        tag = "id",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "JP¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "Rs",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "Dolar Amerika Serikat",
            "EUR" to "Euro",
            "JPY" to "Yen Jepang",
            "GBP" to "Pound Inggris",
            "BRL" to "Real Brasil",
            "CHF" to "Franc Swiss",
            "INR" to "Rupee India",
            "CNY" to "Yuan Tiongkok",
            "RUB" to "Rubel Rusia",
            "EGP" to "Pound Mesir",
            "MXN" to "Peso Meksiko",
            "KRW" to "Won Korea Selatan",
            "TRY" to "Lira Turki",
            "ZAR" to "Rand Afrika Selatan",
            "SAR" to "Riyal Arab Saudi",
            "ARS" to "Peso Argentina",
            "UAH" to "Hryvnia Ukraina",
        ),
        decimal = ",",
        group = ".",
        currencyPattern = "¤#,##0.00",
    ),
    IcuCurrencyGolden(
        tag = "vi",
        symbols = mapOf(
            "USD" to "US\$",
            "EUR" to "€",
            "JPY" to "¥",
            "GBP" to "£",
            "BRL" to "R\$",
            "CHF" to "CHF",
            "INR" to "₹",
            "CNY" to "CN¥",
            "RUB" to "RUB",
            "EGP" to "EGP",
            "MXN" to "MX\$",
            "KRW" to "₩",
            "TRY" to "TRY",
            "ZAR" to "ZAR",
            "SAR" to "SAR",
            "ARS" to "ARS",
            "UAH" to "UAH",
        ),
        names = mapOf(
            "USD" to "Đô la Mỹ",
            "EUR" to "Euro",
            "JPY" to "Yên Nhật",
            "GBP" to "Bảng Anh",
            "BRL" to "Real Braxin",
            "CHF" to "Franc Thụy sĩ",
            "INR" to "Rupee Ấn Độ",
            "CNY" to "Nhân dân tệ",
            "RUB" to "Rúp Nga",
            "EGP" to "Bảng Ai Cập",
            "MXN" to "Peso Mexico",
            "KRW" to "Won Hàn Quốc",
            "TRY" to "Lia Thổ Nhĩ Kỳ",
            "ZAR" to "Rand Nam Phi",
            "SAR" to "Riyal Ả Rập Xê-út",
            "ARS" to "Peso Argentina",
            "UAH" to "Hryvnia Ukraina",
        ),
        decimal = ",",
        group = ".",
        currencyPattern = "#,##0.00 ¤",
    ),
)

/** ICU's independently maintained ISO 4217 numeric-code table. */
internal val icuCurrencyNumericCodes: Map<String, Int> = mapOf(
    "ADP" to 20,
    "AED" to 784,
    "AFA" to 4,
    "AFN" to 971,
    "ALK" to 8,
    "ALL" to 8,
    "AMD" to 51,
    "ANG" to 532,
    "AOA" to 973,
    "AOK" to 24,
    "AON" to 24,
    "AOR" to 982,
    "ARA" to 32,
    "ARP" to 32,
    "ARS" to 32,
    "ARY" to 32,
    "ATS" to 40,
    "AUD" to 36,
    "AWG" to 533,
    "AYM" to 945,
    "AZM" to 31,
    "AZN" to 944,
    "BAD" to 70,
    "BAM" to 977,
    "BBD" to 52,
    "BDT" to 50,
    "BEC" to 993,
    "BEF" to 56,
    "BEL" to 992,
    "BGJ" to 100,
    "BGK" to 100,
    "BGL" to 100,
    "BGN" to 975,
    "BHD" to 48,
    "BIF" to 108,
    "BMD" to 60,
    "BND" to 96,
    "BOB" to 68,
    "BOP" to 68,
    "BOV" to 984,
    "BRB" to 76,
    "BRC" to 76,
    "BRE" to 76,
    "BRL" to 986,
    "BRN" to 76,
    "BRR" to 987,
    "BSD" to 44,
    "BTN" to 64,
    "BUK" to 104,
    "BWP" to 72,
    "BYB" to 112,
    "BYN" to 933,
    "BYR" to 974,
    "BZD" to 84,
    "CAD" to 124,
    "CDF" to 976,
    "CHC" to 948,
    "CHE" to 947,
    "CHF" to 756,
    "CHW" to 948,
    "CLF" to 990,
    "CLP" to 152,
    "CNY" to 156,
    "COP" to 170,
    "COU" to 970,
    "CRC" to 188,
    "CSD" to 891,
    "CSJ" to 203,
    "CSK" to 200,
    "CUC" to 931,
    "CUP" to 192,
    "CVE" to 132,
    "CYP" to 196,
    "CZK" to 203,
    "DDM" to 278,
    "DEM" to 276,
    "DJF" to 262,
    "DKK" to 208,
    "DOP" to 214,
    "DZD" to 12,
    "ECS" to 218,
    "ECV" to 983,
    "EEK" to 233,
    "EGP" to 818,
    "ERN" to 232,
    "ESA" to 996,
    "ESB" to 995,
    "ESP" to 724,
    "ETB" to 230,
    "EUR" to 978,
    "FIM" to 246,
    "FJD" to 242,
    "FKP" to 238,
    "FRF" to 250,
    "GBP" to 826,
    "GEK" to 268,
    "GEL" to 981,
    "GHC" to 288,
    "GHP" to 939,
    "GHS" to 936,
    "GIP" to 292,
    "GMD" to 270,
    "GNE" to 324,
    "GNF" to 324,
    "GNS" to 324,
    "GQE" to 226,
    "GRD" to 300,
    "GTQ" to 320,
    "GWE" to 624,
    "GWP" to 624,
    "GYD" to 328,
    "HKD" to 344,
    "HNL" to 340,
    "HRD" to 191,
    "HRK" to 191,
    "HTG" to 332,
    "HUF" to 348,
    "IDR" to 360,
    "IEP" to 372,
    "ILP" to 376,
    "ILR" to 376,
    "ILS" to 376,
    "INR" to 356,
    "IQD" to 368,
    "IRR" to 364,
    "ISJ" to 352,
    "ISK" to 352,
    "ITL" to 380,
    "JMD" to 388,
    "JOD" to 400,
    "JPY" to 392,
    "KES" to 404,
    "KGS" to 417,
    "KHR" to 116,
    "KMF" to 174,
    "KPW" to 408,
    "KRW" to 410,
    "KWD" to 414,
    "KYD" to 136,
    "KZT" to 398,
    "LAJ" to 418,
    "LAK" to 418,
    "LBP" to 422,
    "LKR" to 144,
    "LRD" to 430,
    "LSL" to 426,
    "LSM" to 426,
    "LTL" to 440,
    "LTT" to 440,
    "LUC" to 989,
    "LUF" to 442,
    "LUL" to 988,
    "LVL" to 428,
    "LVR" to 428,
    "LYD" to 434,
    "MAD" to 504,
    "MDL" to 498,
    "MGA" to 969,
    "MGF" to 450,
    "MKD" to 807,
    "MLF" to 466,
    "MMK" to 104,
    "MNT" to 496,
    "MOP" to 446,
    "MRO" to 478,
    "MRU" to 929,
    "MTL" to 470,
    "MTP" to 470,
    "MUR" to 480,
    "MVQ" to 462,
    "MVR" to 462,
    "MWK" to 454,
    "MXN" to 484,
    "MXP" to 484,
    "MXV" to 979,
    "MYR" to 458,
    "MZE" to 508,
    "MZM" to 508,
    "MZN" to 943,
    "NAD" to 516,
    "NGN" to 566,
    "NIC" to 558,
    "NIO" to 558,
    "NLG" to 528,
    "NOK" to 578,
    "NPR" to 524,
    "NZD" to 554,
    "OMR" to 512,
    "PAB" to 590,
    "PEH" to 604,
    "PEI" to 604,
    "PEN" to 604,
    "PES" to 604,
    "PGK" to 598,
    "PHP" to 608,
    "PKR" to 586,
    "PLN" to 985,
    "PLZ" to 616,
    "PTE" to 620,
    "PYG" to 600,
    "QAR" to 634,
    "RHD" to 716,
    "ROK" to 642,
    "ROL" to 642,
    "RON" to 946,
    "RSD" to 941,
    "RUB" to 643,
    "RUR" to 810,
    "RWF" to 646,
    "SAR" to 682,
    "SBD" to 90,
    "SCR" to 690,
    "SDD" to 736,
    "SDG" to 938,
    "SDP" to 736,
    "SEK" to 752,
    "SGD" to 702,
    "SHP" to 654,
    "SIT" to 705,
    "SKK" to 703,
    "SLE" to 925,
    "SLL" to 694,
    "SOS" to 706,
    "SRD" to 968,
    "SRG" to 740,
    "SSP" to 728,
    "STD" to 678,
    "STN" to 930,
    "SUR" to 810,
    "SVC" to 222,
    "SYP" to 760,
    "SZL" to 748,
    "THB" to 764,
    "TJR" to 762,
    "TJS" to 972,
    "TMM" to 795,
    "TMT" to 934,
    "TND" to 788,
    "TOP" to 776,
    "TPE" to 626,
    "TRL" to 792,
    "TRY" to 949,
    "TTD" to 780,
    "TWD" to 901,
    "TZS" to 834,
    "UAH" to 980,
    "UAK" to 804,
    "UGS" to 800,
    "UGW" to 800,
    "UGX" to 800,
    "USD" to 840,
    "USN" to 997,
    "USS" to 998,
    "UYI" to 940,
    "UYN" to 858,
    "UYP" to 858,
    "UYU" to 858,
    "UYW" to 927,
    "UZS" to 860,
    "VEB" to 862,
    "VED" to 926,
    "VEF" to 937,
    "VES" to 928,
    "VNC" to 704,
    "VND" to 704,
    "VUV" to 548,
    "WST" to 882,
    "XAD" to 396,
    "XAF" to 950,
    "XAG" to 961,
    "XAU" to 959,
    "XBA" to 955,
    "XBB" to 956,
    "XBC" to 957,
    "XBD" to 958,
    "XCD" to 951,
    "XCG" to 532,
    "XDR" to 960,
    "XEU" to 954,
    "XOF" to 952,
    "XPD" to 964,
    "XPF" to 953,
    "XPT" to 962,
    "XSU" to 994,
    "XTS" to 963,
    "XUA" to 965,
    "XXX" to 999,
    "YDD" to 720,
    "YER" to 886,
    "YUD" to 890,
    "YUM" to 891,
    "YUN" to 890,
    "ZAL" to 991,
    "ZAR" to 710,
    "ZMK" to 894,
    "ZMW" to 967,
    "ZRN" to 180,
    "ZRZ" to 180,
    "ZWC" to 716,
    "ZWD" to 716,
    "ZWG" to 924,
    "ZWL" to 932,
    "ZWN" to 942,
    "ZWR" to 935,
)
