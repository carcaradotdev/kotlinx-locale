...
/**
 * The number comparisons that need a golden, next to the goldens they read.
 *
 * See `CountryIcuConformance` for why these are no longer in the shared module.
 * Two independent oracles land here: CLDR's own `@integer` and `@decimal`
 * samples, which say what the data intended, and ICU's answers to the same
 * questions, which were compiled from the same source by code that has never
 * seen this repository's parser. Agreeing with both is a stronger statement than
 ...
 * quietly.
 */
internal fun NumberFormatSource.formatForGolden(setName: String, value: Decimal, locale: Locale): String? = when (setName) {
