...
internal val bundledCurrencyLocales: Set<Locale> by lazy {
    currencyFormatsRegistry.keys.asSequence()
        .filter { it != "root" }
        .mapTo(LinkedHashSet(currencyFormatsRegistry.size)) { Locale.forLanguageTag(it) }
}
 ...
 */
internal fun bundledCurrencySymbol(currencyCode: String, locale: Locale): String? =
    lookupCurrencyName(locale, field = 1, key = currencyCode)

internal fun bundledCurrencyName(currencyCode: String, locale: Locale): String? = lookupCurrencyName(locale, field = 2, key = currencyCode)
