...
val CldrTimeZoneTest by matrixSuite(matrixConfig { testConfig = TestConfig.testScope(isEnabled = false) }) {
    ...
    test("namesAZoneThroughItsMetazone") {
        val losAngeles = zoneOrNull("America/Los_Angeles") ?: return@test
    ...
    test("aZoneWithoutDaylightTimeStillHasBothForms") {
        ...
        val tokyo = zoneOrNull("Asia/Tokyo") ?: return@test
    ...
    test("theLocationFormatNamesTheRegionWhereItHasOneZone") {
    ...
    test("exemplarCitiesAreLocalized") {
        assertEquals("Los Angeles", (zoneOrNull("America/Los_Angeles") ?: return@test).exemplarCity(EN))
        assertEquals("Dubaj", (zoneOrNull("Asia/Dubai") ?: return@test).exemplarCity(CS))
        assertEquals("Praha", (zoneOrNull("Europe/Prague") ?: return@test).exemplarCity(CS))
        assertTrue((zoneOrNull("Europe/Lisbon") ?: return@test).exemplarCity(PT).isNotBlank())
    }

    test("anUnknownZoneStillAnswers") {
        // The fallback ladder ends at the identifier rather than at nothing.
        val zone = zoneOrNull("Etc/GMT+5") ?: return@test
        assertTrue(zone.displayName(locale = EN).isNotBlank())
    }

    test("everyLocaleHasTheOffsetFormat") {
        var checked = 0
